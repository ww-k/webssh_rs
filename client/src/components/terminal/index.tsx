...
export default function Terminal({ active, tab }: { active: boolean; tab: ITab }) {
    ...
    useEffect(() => {
        setTimeout(() => {
            if (!(active && iframeRef.current && iframeRef.current.contentWindow)) {
                return;
            }
            console.log("Terminal active", active);
            iframeRef.current.focus();
            iframeRef.current.contentWindow.focus();
            iframeRef.current.contentWindow.postMessage({ command: "focus" }, "*");
        }, 100);
    }, [active]);

    return <iframe ref={iframeRef} src={url} className="terminalIframe" />;
}
